  ...
  public class Parent: ReceiveActor
  {
    ...
    public Parent()
    {
      route_wokers = Context.ActorOf(
          Worker.Props(1).WithRouter(FromConfig.Instance).WithDispatcher("fork-dispatcher"), "worker_pool1");

      route_wokers2 = Context.ActorOf(
          Worker.Props(2).WithRouter(FromConfig.Instance).WithDispatcher("fork-dispatcher3"), "worker_pool2");

      route_wokers3 = Context.ActorOf(
          Worker.Props(3).WithRouter(FromConfig.Instance).WithDispatcher("fork-dispatcher2"), "worker_pool3");
      ...
      Receive<Response[]>(msgs =>
      {
        ...
        {
          System.Console.WriteLine($"{p.Key, 3} {p.Value, 3} {ThreadCount2[p.Key], 3} {ThreadCount3[p.Key], 3}");
        }
      });
